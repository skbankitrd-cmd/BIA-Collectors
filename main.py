...
async def process_and_save_item(db, anonymizer, llm_processor, item, semaphore):
    """通用的處理與儲存函式 (DRY 原則)"""
    async with semaphore:
        try:
            ...
            if not anonymizer.is_safe_for_cloud(item.body):
                ...
                item.importance = 10
                # 修正：避免全零向量導致 pgvector 崩潰 (Divide by Zero)
                db.insert_intel_item(item, embedding=[1e-5]*768)
                return
            ...
            logger.error(f"單筆處理失敗: {e}", exc_info=True)
...
async def run_pipeline():
    ...
    try:
        ...
        try:
            ...
            # 修正：加入 Semaphore 控制併發數量，防止瞬間打爆 API
            semaphore = asyncio.Semaphore(5)
            tasks = [process_and_save_item(db, anonymizer, llm_processor, item, semaphore) for item in new_items]
